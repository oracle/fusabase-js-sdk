...
/**
 * Integration test: reCAPTCHA Enterprise attestation exchange.
 *
 * Env:
 * - FUSABASE_ORDS_HOST, FUSABASE_PROJECT_ID, FUSABASE_APP_ID
 * - FUSABASE_RECAPTCHA_ENTERPRISE_TOKEN (real token minted by browser/app)
 * - FUSABASE_REDACT_TOKENS=true to redact tokens in logs
 */
describe('AppCheck attestation exchange (integration): recaptcha enterprise', function () {
  this.timeout(60_000);
  ...
  const TOKEN = env('FUSABASE_RECAPTCHA_ENTERPRISE_TOKEN');

  const SKIP = env('FUSABASE_SKIP_APP_CHECK_ATTEST_RECAPTCHA_ENTERPRISE_TEST') === 'true';
  ...
  it('exchanges a token for an fusabaseAttestationToken', async () => {
    ...
    logRest('attest.recaptcha_enterprise', endpoint, init, {
    ...
    if (debugEnabled()) {
      // eslint-disable-next-line no-console
      console.log('[REST][attest.recaptcha_enterprise][response]', {
        status: res.status,
