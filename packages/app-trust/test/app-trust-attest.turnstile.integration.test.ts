...
/**
 * Integration test: Cloudflare Turnstile attestation exchange.
 *
 * Env:
 * - FUSABASE_ORDS_HOST, FUSABASE_PROJECT_ID, FUSABASE_APP_ID
 * - FUSABASE_TURNSTILE_TOKEN (real token minted by browser/app)
 * - FUSABASE_REDACT_TOKENS=true to redact tokens in logs
 */
describe('AppCheck attestation exchange (integration): turnstile', function () {
  this.timeout(60_000);
  ...
  const TOKEN = env('FUSABASE_TURNSTILE_TOKEN');

  const SKIP = env('FUSABASE_SKIP_APP_CHECK_ATTEST_TURNSTILE_TEST') === 'true';
  ...
  it('exchanges a token for an fusabaseAttestationToken', async () => {
    ...
    logRest('attest.turnstile', endpoint, init, {
    ...
    if (debugEnabled()) {
      // eslint-disable-next-line no-console
      console.log('[REST][attest.turnstile][response]', {
        status: res.status,
