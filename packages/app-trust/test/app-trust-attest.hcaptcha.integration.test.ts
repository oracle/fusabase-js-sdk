...
/**
 * Integration test: hCaptcha attestation exchange.
 *
 * Env:
 * - FUSABASE_ORDS_HOST, FUSABASE_PROJECT_ID, FUSABASE_APP_ID
 * - FUSABASE_HCAPTCHA_TOKEN (real token minted by browser/app)
 * - FUSABASE_REDACT_TOKENS=true to redact tokens in logs
 */
describe('AppCheck attestation exchange (integration): hcaptcha', function () {
  this.timeout(60_000);
  ...
  const TOKEN = env('FUSABASE_HCAPTCHA_TOKEN');

  const SKIP = env('FUSABASE_SKIP_APP_CHECK_ATTEST_HCAPTCHA_TEST') === 'true';
  ...
  it('exchanges a token for an fusabaseAttestationToken', async () => {
    ...
    logRest('attest.hcaptcha', endpoint, init, {
    ...
    if (debugEnabled()) {
      // eslint-disable-next-line no-console
      console.log('[REST][attest.hcaptcha][response]', {
        status: res.status,
